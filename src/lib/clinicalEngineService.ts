...
class ClinicalEngineService {
  ...
  private directApiBaseUrl = 'http://127.0.0.1:8000';

  /**
   * NEW: Extract observations from a transcript using an LLM (mocked for now)
   */
  async extractObservationsFromTranscript(transcript: string): Promise<string[]> {
    console.log("Calling mock extractObservationsFromTranscript with transcript:", transcript.substring(0, 100) + "...");
    await new Promise(resolve => setTimeout(resolve, 750)); // Simulate API call
    const observations: string[] = [];
    const lowerTranscript = transcript.toLowerCase();

    // Simple keyword-based mock extraction
    if (lowerTranscript.includes("headache")) observations.push("headache");
    if (lowerTranscript.includes("fever")) observations.push("fever");
    if (lowerTranscript.includes("nausea")) observations.push("nausea");
    if (lowerTranscript.includes("dizziness")) observations.push("dizziness");
    if (lowerTranscript.includes("fatigue")) observations.push("fatigue");
    if (lowerTranscript.includes("joint pain")) observations.push("joint pain");
    if (lowerTranscript.includes("cough")) observations.push("cough");
    if (lowerTranscript.includes("sore throat")) observations.push("sore throat");

    if (observations.length === 0 && lowerTranscript.length > 10) {
      // If no specific keywords, add a generic observation if transcript is not trivial
      observations.push("general malaise reported");
    }

    console.log("Mock extracted observations:", observations);
    return observations;
  }
  ...
  async executeDiagnosticPlan(plan: DiagnosticPlan, patientId?: string, updateCallback?: (plan: DiagnosticPlan) => void): Promise<DiagnosticPlan> {
    // This is also a more granular step than /run-dx. Keeping mock.
    // TODO: call actual Tool B backend here when available.
    try {
      // Simulate API call delay and step-wise execution
      ...
      for (const step of plan.steps) {
        ...
        if (updateCallback) {
          // Create a new plan object for the callback to reflect current state
          const currentPlanState: DiagnosticPlan = { 
            ...plan, // Spread existing plan properties like rationale
            steps: [...updatedSteps, ...plan.steps.slice(updatedSteps.length)] 
          };
          updateCallback(currentPlanState);
        }
        // Simulate delay between steps if not the last step
        if (updatedSteps.length < plan.steps.length) {
   ...
   * This method now calls the /run-dx backend endpoint.
   */
  async generateDiagnosticResult(
    patientId: string, 
    transcript: string, 
    patientDataDict: Record<string, any>,
    observations?: string[], // Changed from symptoms to observations
    plan?: DiagnosticPlan // Optional: plan is generated by backend within /run-dx
  ): Promise<ClinicalOutputPackage> {
    console.log("Calling /run-dx with:", { patientId, transcript, patientDataDict, observations }); // observations added
    try {
      const response = await fetch(`${this.directApiBaseUrl}/run-dx`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ 
          ...
          patient_data_dict: patientDataDict,
          observations: observations // Changed from symptoms
        }),
      });

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ detail: response.statusText }));
        console.error('Error from /run-dx API:', errorData);
        throw new Error(`Failed to generate diagnostic result: ${errorData.detail || 'Unknown error'}`);
      }

      const result: ClinicalOutputPackage = await response.json();
      console.log("Received result from /run-dx:", result);
      return result;

    } catch (error) {
      console.error('Error calling /run-dx or processing its response:', error);
      console.warn('Falling back to mock data for generateDiagnosticResult');
      return this.getMockClinicalOutputPackage(patientId, observations || [], plan); // Changed from symptoms
    }
  }

  // Helper for mock data generation (extracted from original method)
  private getMockClinicalOutputPackage(patientId: string, observations: string[], plan?: DiagnosticPlan): ClinicalOutputPackage { // Changed from symptoms
      // This needs to return a full ClinicalOutputPackage, including a mock DiagnosticResult
      // and optionally mock SOAP notes, etc.
      const mockDiagResult = this.getMockDiagnosticResult(observations, plan); // Changed from symptoms

      // Create a basic mock SoapNote
      const mockSoapNote: SoapNote = {
        subjective: "Patient reports mock symptoms.",
        objective: "Mock objective findings.",
        assessment: `Mock assessment: ${mockDiagResult.diagnosisName}`,
        plan: "Mock treatment plan details.",
        rawTranscriptSnippet: "Mock transcript snippet."
      };
      ...
      return {
        ...
        patientId: patientId,
        diagnosticResult: mockDiagResult,
        soapNote: mockSoapNote, // Include a mock SOAP note
        // referralDocument and priorAuthDocument can be omitted or mocked minimally
        referralDocument: undefined, 
        priorAuthDocument: undefined,
        evidenceSources: [
          {
            type: "guideline",
            id: "mock_guideline_source_1",
            title: "Mock Guideline Source",
            content: "This is content from a mock guideline source.",
            accessTime: new Date().toISOString(),
            relevanceScore: 0.75
          }
        ]
  ...
  private getMockDiagnosticResult(observations: string[], plan?: DiagnosticPlan): DiagnosticResult { // Changed from symptoms
      // Simulate API call delay
      // await new Promise(resolve => setTimeout(resolve, 2000)); // Delay already handled by caller if it was mock-only path
      
      // Check for patterns in the findings to determine diagnosis (if plan is available)
      const hasRheumatoidArthritis = plan?.steps.some(step => 
        step.findings.toLowerCase().includes('rheumatoid arthritis')) || observations.includes("rheumatoid arthritis"); // Changed from symptoms
      
      const hasLeukemia = plan?.steps.some(step => 
        step.findings.toLowerCase().includes('leukemia')) || observations.includes("leukemia"); // Changed from symptoms
      
      if (hasRheumatoidArthritis) {
        // RA diagnosis
        return {
          diagnosisName: "Rheumatoid Arthritis (Mock)",
        ...
        // Leukemia diagnosis
        return {
          diagnosisName: "Chronic Myeloid Leukemia (Mock)",
        ...
        // Generic result
        return {
          diagnosisName: "Undifferentiated Inflammatory Condition (Mock)",
          diagnosisCode: "M06.9",
          confidence: 0.65,
          supportingEvidence: [
