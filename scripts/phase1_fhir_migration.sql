...
-- 3. Add status field to encounters (FHIR Encounter.status)
ALTER TABLE public.encounters 
  ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'finished';

-- 4. Add is_deleted boolean to encounters for soft deletes
...
CREATE TABLE IF NOT EXISTS public.conditions (
  ...
  encounter_id UUID REFERENCES public.encounters(id),  -- which encounter generated or noted this diagnosis (nullable for chronic conditions)
...
JOIN public.encounters v ON v.patient_supabase_id = p.id
