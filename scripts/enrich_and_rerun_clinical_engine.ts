...
async function callClinicalEngineAPI(patientId: string, encounterId: string, transcript: string) {
  const body = { patientId, encounterId, transcript };
  const resp = await fetch(`${API_BASE_URL}/api/clinical-engine`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body)
  });
  if (!resp.ok) {
    throw new Error(`Clinical engine API error ${resp.status}: ${await resp.text()}`);
  }
  return resp.json();
}
